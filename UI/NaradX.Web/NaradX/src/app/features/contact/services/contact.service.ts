...
export class ContactService {
  ...
  getContacts(filters: ContactFilters): Observable<PagedResult<ContactDto>> {
    return this.api.post<PagedResult<ContactDto>>('api/contact/list', filters); 
  }

  addContact(contact: ContactDto): Observable<number> {
    return this.api.post('api/contact/add', contact);
  }

  updateContact(contact: ContactDto): Observable<number> {
    return this.api.put('api/contact/update', contact);
  }
  ...
  deleteContact(id: number): Observable<number> {
    ...
    return this.api.get(`api/contact/delete-contact-by-id/${id}`);
  }

  getContactById(id: number): Observable<ContactDto> {
      return this.api.get(`api/contact/get-contact-by-id/${id}`);
  }
  ...
  validateBulkUpload(file: File, countryId: number, languageId: number, source: string, channel: string): Observable<BulkUploadValidateResponseDto> {
    ...
    return this.api.postMultipart('api/contact/bulk-upload-validate', formData);
  }

  saveBulkUpload(data: BulkUploadValidateResponseDto): Observable<any> {
      return this.api.post('api/contact/bulk-upload-confirm', data);
  }

  downloadTemplate(): Observable<Blob> {
      return this.api.getFile('api/contact/bulk-upload-download-template');
  }
}
