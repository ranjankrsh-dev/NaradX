...
namespace NaradX.API.Extensions
{
    public static class InfrastructureServiceExtensions
    {
        public static IServiceCollection AddInfrastructureServices(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            // Register WhatsApp API client with logging
            services.AddRefitClient<IWhatsAppApiGateway>(refitSettings)
                .ConfigureHttpClient(c => {
                    c.BaseAddress = new Uri(whatsAppOptions.BaseUrl);
                    c.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", whatsAppOptions.AccessToken);
                })
                .ConfigurePrimaryHttpMessageHandler(sp => new LoggingHttpMessageHandler(sp.GetRequiredService<ILogger<LoggingHttpMessageHandler>>()));
